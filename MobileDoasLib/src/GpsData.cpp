...
static unsigned char CalculateMessageChecksum(const std::string& message)
{
    if (message.size() == 0)
    {
        return 0; // invalid
...
static bool ChecksumIsCorrect(const std::string& message)
{
    const size_t startOfChecksum = message.find("*");
...
static void SetDate(const std::string& curToken, GpsData& data)
{
    if (curToken.length() != 6)
    {
...
static double ConvertToDecimalDegrees(double rawData)
{
...
static void SetLatitude(const std::string& curToken, GpsData& data)
{
...
static void SetLongitude(const std::string& curToken, GpsData& data)
{
    if (curToken.length() < 3)
    {
        return; // invalid format.
...
static void SetHemisphere(const std::string& curToken, GpsData& data)
{
    if (curToken.size() != 1)
    {
...
static void SetNumberOfSatellitesTracked(const std::string& curToken, GpsData& data)
{
...
static void SetNumberOfSatellitesSeen(const std::string& curToken, GpsData& data)
{
    if (curToken.size() < 1 || curToken.size() > 2)
    {
        return; // not enough information
...
static void SetFixQuality(const std::string& curToken, GpsData& data)
{
...
static void SetAltitude(const std::string& curToken, GpsData& data)
{
    if (curToken.size() < 1)
    {
        return; // not enough information
...
static void SetAltitudeUnit(const std::string& curToken, GpsData& /*data*/)
{
    if (curToken.size() != 1)
    {
        return; // not enough information
...
static void SetStatus(const std::string& curToken, GpsData& data)
{
...
static void SetSpeed(const std::string& curToken, GpsData& data)
{
...
static void SetCourse(const std::string& curToken, GpsData& data)
{
    if (curToken.length() == 5)
    {
...
static bool ExtractMessage(const char* gpsString, const char* messageType, std::string& result)
{
    ...
    // The message starts with the given gpsString (e.g. $GPRMC) and ends with a carriage return / line feed
...
// Parses the RMC string (starting with $GPRMC or $GNRMC) and fills in the provided GpsData structure
// The $GPRMC is used by the GPS system and $GNRMC by other satellite navigation systems.
// Lists Position, Velocity, and Time.
// @return true if the parsing is successful
static bool ParseRMC(const char* gpsString, GpsData& data)
{
    ...
    {
        if (!ExtractMessage(gpsString, "$GNRMC", message))
        {
            return false;
        }
    }
...
// Parses the GGA string (starting with $GPGGA or $GNGGA) and fills in the provided GpsData structure
// The GPGGA is used by the GPS system and GNGGA by other satellite navigation systems.
// @return true if the parsing is successful
static bool ParseGGA(const char* gpsString, GpsData& data)
{
    const std::string delimiter = ",";
    ...
    {
        if (!ExtractMessage(gpsString, "$GNGGA", message))
        {
            return false;
        }
    }
...
// This message is issued by the (US) GPS system to list number of sattelites in view.
// @return true if the parsing is successful
static bool ParseGSV(const char* gpsString, GpsData& data)
{
    const std::string delimiter = ",";
