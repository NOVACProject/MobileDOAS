...
//	m_gpsPlot.SetGridColor(RGB(255,255,255));
	...
	for(int i = 0; i < m_traversesToShow; ++i){
		this->m_evalLogList.AddString(m_flux->m_traverse[i]->m_fileName);
	}
...
void CRouteDlg::DrawRouteGraph(){
	...
	if(1 == m_showScale){
		...
		double plotwidth			= 1e-3 * GPSDistance(m_gpsPlot.GetYMin(), m_gpsPlot.GetXMin(), m_gpsPlot.GetYMin(), m_gpsPlot.GetXMax());
		...
		CalculateDestination(sLat[0], sLong[0], scaleWidth, 90, sLat[1], sLong[1]);
...
LRESULT CRouteDlg::OnLBUInGPSGraph(WPARAM wParam, LPARAM lParam){
	...
	if(1 == m_calcDistance.GetCheck()){
		...
		bearing = GPSBearing(lat1, lon1, lat2, lon2);
...
void CRouteDlg::GetPlotRange(struct plotRange &range){
	...
	for(i = 0; i < m_traversesToShow; ++i){
		...
		range.maxLat = std::max(Max(m_lat[i], m_recordLen[i]), range.maxLat);
		range.maxLon = std::max(Max(m_lon[i], m_recordLen[i]), range.maxLon);
		range.minLat = std::min(Min(m_lat[i], m_recordLen[i]), range.minLat);
		range.minLon = std::min(Min(m_lon[i], m_recordLen[i]), range.minLon);
		}
	}
...
void CRouteDlg::GetColorRange(double &minValue, double &maxValue){
	...
	for(i = 0; i < m_traversesToShow; ++i){
		...
		switch(m_selectedDisplay){
		case SHOW_INTENSITY:
			minValue = std::min(Min(m_int[i], m_recordLen[i]), minValue);
			maxValue = std::max(Max(m_int[i], m_recordLen[i]), maxValue);
			break;
		case SHOW_ALTITUDE:
			minValue = std::min(Min(m_alt[i], m_recordLen[i]), minValue);
			maxValue = std::max(Max(m_alt[i], m_recordLen[i]), maxValue);
			break;
		case SHOW_COLUMN:
			minValue = std::min(Min(m_col[i], m_recordLen[i]), minValue);
			maxValue = std::max(Max(m_col[i], m_recordLen[i]), maxValue);
			break;
		default:
			minValue = std::min(Min(m_col[i], m_recordLen[i]), minValue);
			maxValue = std::max(Max(m_col[i], m_recordLen[i]), maxValue);
...
void CRouteDlg::InitBuffers(){
	...
	for(i = 0; i < m_traversesToShow; ++i){
		Flux::CTraverse *tr = m_flux->m_traverse.GetAt(i);
		...
		memcpy(m_col[i], tr->columnArray, tr->m_recordNum*sizeof(double));
