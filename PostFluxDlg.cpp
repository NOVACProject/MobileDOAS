...
#include "Common/KMLFileHandler.h"
...
int CPostFluxDlg::OpenLogFile(CString &filename, CString &filePath){
	...
	if(UpdateData(TRUE)){
		...
		fileType = m_flux->ReadSettingFile(filename, nChannels, fileVersion);
		if(fileType!=1){
		...
		for(int k = 1; k <= nChannels; ++k){
			CTraverse *tr = new CTraverse();
			m_flux->m_traverse.SetAtGrow(m_flux->m_traverseNum + k, tr);
		}

		// Read the data from the file
		int oldNumberOfFilesOpened = m_flux->m_traverseNum;
		if(0 == m_flux->ReadLogFile(filePath, filename, nChannels, fileVersion)){
		...
		for(int k = 0; k < nChannels; ++k){
			tmpStr.Format(m_flux->m_traverse[oldNumberOfFilesOpened + k]->m_fileName);
...
{
	CString fluxMessage, fluxRangeMessage, strMessage,str1,str2,str3, fileName;
...
void CPostFluxDlg::ShowColumn()
{
	...
	if(m_XAxisUnit == 2){
		...
		for(k = 1; k < m_right-m_left+1; ++k){
			distance[k] = distance[k-1] + 1e-3 * GPSDistance(latBuffer[k], lonBuffer[k], latBuffer[k-1], lonBuffer[k-1]);
		}
		if(distance[k-1] < 1.0){
...
void CPostFluxDlg::UpdatePropertiesWindow(){
	CTraverse *cur = m_flux->m_traverse[m_flux->m_curTraverse];
...
void CPostFluxDlg::OnReEvaluateThisTraverse()
{
	...
	if(reEvalDlg.m_reeval->fRun){
		...
		if(reEvalDlg.m_page5.pReEvalThread != NULL){
...
void CPostFluxDlg::OnFileExportToKML(){
	...
	if(Common::BrowseForFile_SaveAs("*.kml", fileName)){
		...
		// Store the file
		FileHandler::CKMLFileHandler::StoreTraverseAsKML(*m_flux->m_traverse[m_flux->m_curTraverse], fileName, scalingHeight);
	}
}
