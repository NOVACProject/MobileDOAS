...
void CReEval_FitWindowsDlg::PopulateReferenceFileControl(){
	...
	for(i = 0; i < window.nRef; ++i){
		...
		Evaluation::CReferenceFile &ref = window.ref[i];
		...
		if(ref.m_shiftOption == Evaluation::SHIFT_OPTIMAL)
		...
		if(ref.m_squeezeOption == Evaluation::SHIFT_OPTIMAL)
...
void CReEval_FitWindowsDlg::OnInsertReference(){
	...
	if(window.nRef == 0){
		...
		window.ref[window.nRef].m_shiftOption   = Evaluation::SHIFT_OPTIMAL;
		window.ref[window.nRef].m_squeezeOption = Evaluation::SHIFT_OPTIMAL;
	}else{
		...
		window.ref[window.nRef].m_squeezeValue  = 0;
	}
