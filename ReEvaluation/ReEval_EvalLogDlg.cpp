...
void CReEval_EvalLogDlg::DrawColumn(){
	...
	if(m_reeval->m_recordNum[0] < MAX_TRAVERSE_SHOWN){
		m_right = m_reeval->m_recordNum[0]-1;
	}else{
	...
	double *intens = new double[m_right - m_left + 1];
	memcpy(intens, m_reeval->m_int[0], (m_right-m_left+1)*sizeof(double));
	double dynRange_inv = 100.0 / m_reeval->m_spectrometerDynRange;
	for(int k = 0; k < m_right-m_left+1; ++k)
		intens[k] = intens[k] * dynRange_inv;

	/* Draw the column */
	m_ColumnPlot.Plot(m_reeval->m_oldCol[0], (m_right-m_left+1));

	/* draw the intensity */
	m_ColumnPlot.DrawCircles(number, intens, (int)(m_right-m_left+1), Graph::CGraphCtrl::PLOT_SECOND_AXIS);
