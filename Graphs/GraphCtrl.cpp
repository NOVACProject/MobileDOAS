...
void CGraphCtrl::InvalidateCtrl()
{
    ...
    if (m_axisOptions.drawRightUnits)
    {
        ...
        size_t nCharacters = strlen(strTemp);
        ...
        m_rectPlot.right = m_rectClient.right - 6 * (long)nCharacters - m_axisOptions.unitFont.height;
    }
    else
    {
...
void CGraphCtrl::ShadeFilledSquare(double x_min, double x_max, double y_min, double y_max, double shade) {
    ...
    for (int i = rect.left; i < rect.right; ++i) {
        for (int j = rect.bottom; j < rect.top; ++j) {
            COLORREF clr = m_dcPlot.GetPixel(i, j);
...
void CGraphCtrl::BarChart2(double* xPosition, double* yValues1, double* yValues2, double* yError1, double* yError2, COLORREF color, int pointSum, int plotOption)
{
    ...
    for (int i = 0; i < pointSum; ++i) {
        if (fabs(yValues1[i]) > fabs(yValues2[i])) {
            ...
            extremeErr[i] = (yError1 != nullptr) ? yError1[i] : 0;
            extremeCol[i] = m_colors.plot;
            smallV[i] = yValues2[i];
            smallVErr[i] = (yError2 != nullptr) ? yError2[i] : 0;
            smallCol[i] = color;
        }
        else {
            extreme[i] = yValues2[i];
            extremeErr[i] = (yError2 != nullptr) ? yError2[i] : 0;
            extremeCol[i] = color;
            smallV[i] = yValues1[i];
            smallVErr[i] = (yError1 != nullptr) ? yError1[i] : 0;
