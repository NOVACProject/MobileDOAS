	...
	for(i = 0; i < pointSum; ++i)
	{
		...
		if(plotOption & PLOT_CIRCLES){
			if((plotOption & PLOT_SCALE_CIRCLE) && color != NULL){
				r = max(2, 15 * (color[i] - minC) / (maxC - minC));
...
void CGraphCtrl::BarChart(double *xPosition, double *yValues, double *yErrors, int pointSum, int plotOption)
{
  ...
  m_dcPlot.SetTextColor(m_colors.grid) ;
	...
	for(i = 0; i < pointSum; ++i){
		...
		if(yErrors != NULL){
			curYError[0] = (int)(topLefty - (yValues[i] - yErrors[i] - offsBottom) * yFactor);
			curYError[1] = (int)(topLefty - (yValues[i] + yErrors[i] - offsBottom) * yFactor);

			int curX		 = m_rectPlot.left + X;
			// Draw the y-error bar
			m_dcPlot.MoveTo(curX - 2,	curYError[0]);
			m_dcPlot.LineTo(curX + 2,	curYError[0]);
			m_dcPlot.MoveTo(curX,			curYError[0]);
