...
void CConfigure_Evaluation::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_FITFROM, m_fitLow);
    DDX_Text(pDX, IDC_EDIT_FITTO, m_fitHigh);
    DDX_Text(pDX, IDC_REEVAL_WINDOW_POLYNOM2, m_polyOrder);
    DDX_CBIndex(pDX, IDC_COMBO_CHANNEL, m_fitChannel);
    DDX_Check(pDX, IDC_CHECK_STARTMEASUREMENTBYCALIBRATION, m_runCalibrationInMeasurement);
...
void CConfigure_Evaluation::PopulateWindowList() {
    m_windowList.ResetContent(); // clear the list
...
void CConfigure_Evaluation::SaveData()
{
    ...
    m_conf->m_calibration.m_enable = (m_runCalibrationInMeasurement != 0);
    ...
    window.fitLow = m_fitLow;
    window.fitHigh = m_fitHigh;
    window.polyOrder = m_polyOrder;
    window.channel = m_fitChannel;
}

void CConfigure_Evaluation::OnChangeFitWindow()
{
    m_runCalibrationInMeasurement = (m_conf->m_calibration.m_enable) ? 1 : 0;
    ...
    // Update the controls
    m_fitHigh = m_conf->m_fitWindow[curWindow].fitHigh;
    m_fitLow = m_conf->m_fitWindow[curWindow].fitLow;
    m_polyOrder = m_conf->m_fitWindow[curWindow].polyOrder;
    m_fitChannel = m_conf->m_fitWindow[curWindow].channel;
