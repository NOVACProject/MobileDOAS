...
double CTraverse::GetTotalFlux(double windSpeed, double windDirection){
	...
	Common common;
	CString str1,str2,tmpStr,fluxFileName,strEvlog;
	long i;
...
double CTraverse::CalculateFlux(double column, const gpsPosition &pos1, const gpsPosition &pos2, double windSpeed, double windDirection)
{
	...
	double  windFactor	= GetWindFactor(lat1,lon1,lat2,lon2, windDirection);
...
{	
	...
	memcpy((void*)tmpColumn, (void*)columnArray, sizeof(double)*m_recordNum);
	sumColumn = 0.0;
	maxColumn = 0.0;
	avIndex	  = 0;
	maxIndex  = 0;
	...
	maxBuffer[3]=GPSBearing(maxLat, maxLon, srclat,srclon);
	...
	avBuffer[3]=GPSBearing(avLat,avLon, srclat,srclon);
...
double CTraverse::CalculateOffset(){
	...
	double *testColumns = new double[m_recordNum];
	...
	for(i = 0; i < m_recordNum; ++i){
		...
		testColumns[numColumns++] = columnArray[i];
	}
	...
	this->m_Offset = CalculateOffset(testColumns, numColumns);
...
/** Calculates the offset of the given set of column data
	@return the offset value. This will be 0.0 if nothing 
		could be calculated */
double CTraverse::CalculateOffset(double *columnData, long nDataPoints) const{
	double avg = 0.0;
	...
	if(FindNLowest(columnData, nDataPoints, m, N)){
		avg = Average(m, N);
	}else{
		avg = 0.0;
	}
...
CTraverse &CTraverse::operator=(const CTraverse &t){
	...
	this->m_fileName.Format(t.m_fileName);
	this->m_filePath.Format(t.m_filePath);
	this->m_specieName.Format(t.m_specieName);
	...
	memcpy(this->columnArray, t.columnArray, MAX_TRAVERSELENGTH*sizeof(double));
	memcpy(this->columnError, t.columnError, MAX_TRAVERSELENGTH*sizeof(double));
	memcpy(this->intensArray, t.intensArray, MAX_TRAVERSELENGTH*sizeof(double));
	memcpy(this->time, t.time, MAX_TRAVERSELENGTH*sizeof(Time));
