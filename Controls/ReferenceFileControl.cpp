...
void CReferenceFileControl::OnEndEditCell(int nRow, int nCol, CString str){
	...
	if(nCol == 2){
		...
		case SHIFT_FIX:     SetItemTextFmt(nRow, 2, "fix to %.2lf", ref.m_shiftValue); break;
		case SHIFT_LINK:    SetItemTextFmt(nRow, 2, "link to %s", m_window->ref[(int)ref.m_shiftValue]); break;
		case SHIFT_OPTIMAL: SetItemTextFmt(nRow, 2, "find optimal"); break;
		}
	}
	...
	if(nCol == 3){
		...
		case SHIFT_FREE:    SetItemTextFmt(nRow, 2, "free"); break;
		case SHIFT_FIX:     SetItemTextFmt(nRow, 2, "fix to %.2lf", ref.m_squeezeValue); break;
		case SHIFT_LINK:    SetItemTextFmt(nRow, 2, "link to %s", m_window->ref[(int)ref.m_squeezeValue]); break;
		case SHIFT_OPTIMAL: SetItemTextFmt(nRow, 2, "find optimal"); break;
		}
	}
